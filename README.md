# Learning Projects — 12 проектов на Python

Двенадцать учебных проектов: 11 Telegram-ботов по техническому заданию (файл `1.txt` в этом репозитории) и «Умный Шоппер» — ИИ-ассистент покупок с Mini App. Каждый проект размещён **в отдельном репозитории** — для независимого использования и дальнейшего развития. Данный репозиторий содержит индекс, ТЗ, валидатор и сводный дамп кода.
...
| 11 | [marketplace-price-compare](https://github.com/d3c0r1x/marketplace-price-compare) | Сравнение цен WB, Ozon и Яндекс Маркет: поиск, лучшая цена, watch-подписки на падение цены | aiogram v3, httpx, curl_cffi, aiosqlite, apscheduler, pydantic |
| 12 | [smart-shopper](https://github.com/d3c0r1x/smart-shopper) | ИИ-ассистент покупок для Ozon и Яндекс Маркета: текстовый поиск, поиск по фото, анализ отзывов, сравнение цен. Telegram-бот + Mini App | aiogram v3, aiohttp, openai (OpenRouter), pydantic, aiosqlite, React + Vite |
...
- папки `project1_...`–`project12_...` — рабочие копии, каждая со своим git-репозиторием и GitHub Actions CI.
...
Итого **169 unit-тестов** (12+9+9+7+12+9+8+9+7+19+29+39) + изолированный валидатор. В Проектах 5, 10, 11 и 12 — интеграционные тесты полных игровых/рабочих сценариев через настоящий Dispatcher с перехватом исходящих вызовов Bot API. В каждом README — раздел «Технические особенности» с описанием выявленных ограничений и решений, а также «Планы развития».
...
7. **OpenRouter** (Проект 12) — только `:free`-модели: лимиты 20 запросов/мин и 50/день (после пополнения от $10 — 1000/день); модели регулярно снимаются — реестр с fallback-цепочками и роутер `openrouter/free`.
8. **Ozon / Яндекс Маркет** (Проект 12) — официальные API предназначены для продавцов; данные каталога и отзывов — через публичные web-эндпоинты с демо-режимом по умолчанию.
...
- **1, 10, 11**: трекер WB, трекер Ozon и компаратор WB↔Ozon↔Яндекс образуют линейку инструментов для работы с маркетплейсами (адаптеры WB в 1 и 11 построены на одном эндпоинте `search.wb.ru`);
...
- сравнение по конкретному артикулу (не только по текстовому запросу).
